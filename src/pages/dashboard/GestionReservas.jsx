...
export default function GestionReservas() {
  ...
  const handleDelete = (id) => async () => {
    ...
    if (result.status === 200) {
      setModal(true);
      setModalTitle("La reserva se ha eliminado");
      setModalMessage("La reserva se ha eliminado correctamente.");
    ...
    <div className="p-10 flex flex-col gap-5 w-full">
      {modal && (
        <Modal
          ...
          pathNavigate="/dashboard/gestionar-reservas"
          setModal={setModal}
        />
      )}
            ...
            <TableRow key={item.id}>
              ...
              <TableCell>
                <div className="flex flex-col gap-1 text-slate-50 text-center text-xs font-bold">
                  <Link
                    to={`/dashboard/gestionar-reservas`}
                  ...
                  <Link
                    to={`/dashboard/gestionar-reservas/detalles/${item.id}`}
                    className="bg-slate-900 p-1 rounded-lg"
                  >
                    Detalles
                  </Link>
